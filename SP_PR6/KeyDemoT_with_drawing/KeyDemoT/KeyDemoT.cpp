...
HWND Create(HINSTANCE hInstance, int nCmdShow)
{
	...
	g_hDC = GetDC(hwnd); // Контекст для вывода трассы мыши
...
void km_OnDestroy(HWND hwnd)
{
	ReleaseDC(hwnd, g_hDC); // Освобождение контекста вывода трассы
	PostQuitMessage(0);
}
...
void km_OnChar(HWND hwnd, UINT ch, int cRepeat)
{
	...
	ReleaseDC(hwnd, DC);//Освобождаем локальный контекст
}

//=== Обработчик сообщения WM_KEYUP,WM_KEYDOWN =======================//
void km_OnKey(HWND hwnd, UINT vk, BOOL fDown, int cRepeat, UINT flags)
{
	TCHAR S[100]; //Буфер для формирования выводимой строки(100 символов)
	HDC DC = GetDC(hwnd); //Получаем контекст устройства графического вывода
	...
	int y, UINT keyFlags)
{
	...
	if (fDoubleClick) //Двойной щелчек
		...
		fDraw = TRUE; // Включить вывод трассы мыши
		MoveToEx(g_hDC, x, y, NULL); // Установка начальной позиции пера
		SetCapture(hwnd);	// Захват сообщений мыши для окна hwnd
...
void km_OnLButtonUp(HWND hwnd, int x, int y, UINT keyFlags)
{
	...
	fDraw = FALSE;  // Отключить вывод трассы
	ReleaseCapture();// Отмена захвата мыши для окна hwnd
}
...
void km_OnMouseMove(HWND hwnd, int x, int y, UINT keyFlags)
{
	...
	if (fDraw) {
		// Отметка следа мыши
		wsprintf(S, TEXT("След: x = %d, y = %d"), x, y);
		SetBkColor(DC, RGB(255, 255, 255)); // Белый фон
		TextOut(DC, x, y, S, lstrlen(S)); // Вывод следа
	}

	// Вывод текущих координат мыши в верхнем левом углу
	SetBkColor(DC, GetSysColor(COLOR_WINDOW)); // Цвет фона
	wsprintf(S, TEXT("x = %d, y = %d"), x, y);
	TextOut(DC, 0, 0, S, lstrlen(S)); // Вывод координат
	ReleaseDC(hwnd, DC);
	...
	if (fDraw)
		LineTo(g_hDC, x, y); // Вывод линии в глобальный контекст
	//	LineTo(DC,x,y);
...
void km_OnPaint(HWND hwnd)
{
	...
	static TCHAR lpszTitle1[] = TEXT("ДЕМОНСТРАЦИЯ ПЕРЕХВАТА ВВОДА МЫШИ И КЛАВИАТУРЫ");
	static TCHAR lpszTitle2[] = TEXT("Поэкпериментируйте с мышью и клавиатурой");

	//Массив указателей на строки инициализации экрана 
	
	static const TCHAR* SList[6]= //;
	//SList[0] = TEXT("WM_CHAR     ");
	{ TEXT("WM_CHAR     "),
	TEXT("WM_KEY      "),
	TEXT("WM_SYSKEY   "),
	TEXT("WM_MOUSEMOVE"),
	...
	SetBkColor(PaintDC, GetSysColor(COLOR_WINDOW));//Задаем цвет фона
